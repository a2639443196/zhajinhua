"""
 ...
 Description: 核心牌型与数据结构 (源自之前的 game.py)
 (已修改：实现花色优先级)
"""
import random
...
def evaluate_hand(cards: list[Card]) -> HandRank:
    ...
    if rank_set == a23_ranks:
        is_straight = True
        # A-2-3 是最小的顺子，其 key 以 3 (即 A23 的 A) 为准... 不，A23 最小，以 3 为准
...
def compare_hands(cards_a: list[Card], cards_b: list[Card]) -> int:
    ...
    # (新) 规则：若完全一致（包括花色），主动比牌者负
    # (此逻辑在 zhajinhua.py 的 _do_compare 中处理，这里返回 0 即可)
    return 0
